                    ...
                    <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                      ...
                      {!isStaff && !isReadOnly && (
                        <div style={{ display: 'flex', gap: 4, flexShrink: 0, alignSelf: 'center' }}>
                          ...
                          <button
                            onClick={() => {
                              const updated = order.payments.filter((x) => x.id !== p.id);
                              addPaymentMutation.mutate({ orderId: order.id, payments: updated, totalAmount: order.totalAmount });
                            }}
                    ...
                    {editingPaymentId === p.id && (
                      <div style={{ marginLeft: 20, marginTop: 6, marginBottom: 8, padding: '10px 12px', background: isDark ? 'rgba(255,255,255,0.03)' : T.bg2, borderRadius: T.r.sm, border: `1px solid ${T.border}`, display: 'flex', flexDirection: 'column', gap: 8 }}>
                        <div style={{ display: 'flex', gap: 8 }}>
                          ...
                          <div style={{ flex: 1 }}>
                            <DateInput label="Date" value={editPDate} onChange={setEditPDate} padding="7px 12px" />
                          </div>
                        </div>
                        <div>
                        ...
                        <div style={{ display: 'flex', gap: 8 }}>
                          ...
                          <button
                            ...
                            onClick={async () => {
                              const updated = order.payments.map((x) =>
